...
createButton("Бесконечный прыжок", function()
    infjump = not infjump
    if infjump and not infJumpConnection then
        infJumpConnection = UIS.JumpRequest:Connect(function() hum:ChangeState("Jumping") end)
    elseif not infjump and infJumpConnection then
...
createButton("Noclip", function() noclip = not noclip end)
...
local rainbow = false
createButton("Радужный", function()
    rainbow = not rainbow
    spawn(function()
        while rainbow do
...
createButton("Авто-кликер", function()
